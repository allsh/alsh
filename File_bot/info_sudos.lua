...
if keko_add_sudo then var = true end return var
    ...
    if not database:get('tshake:'..bot_id..'akeko:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_) then 
    database:incrby('tshake:'..bot_id..'keko:sudo'..bot_id..msg.sender_user_id_, 1)
    database:set('tshake:'..bot_id..'akeko:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_, "keko")
    end  
    end
    end
    ...
    if not database:get('tshake:'..bot_id..'aakeko:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_) then 
    database:incrby('tshake:'..bot_id..'keko:sudo2'..bot_id..msg.sender_user_id_, 1)
    database:set('tshake:'..bot_id..'aakeko:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_, "keko")
    end  
    end
    end
    ...
    local sudo_keko = redis:smembers('tshake:'..bot_id..'dev')
    local keko = "💬┇احصائيات المطورين : \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n\n"
    local keko2 = database:get('tshake:'..bot_id..'keko:sudo'..bot_id..sudo_add)
    local keko3 = database:get('tshake:'..bot_id..'keko:sudo2'..bot_id..sudo_add)
    ...
    local user_info = database:hgetall('tshake:'..bot_id..'user:'..sudo_keko[i])
    local username = nil
    if user_info and user_info.username then
    username = "@"..user_info.username
    end
    local keko2 = database:get('tshake:'..bot_id..'keko:sudo'..bot_id..sudo_keko[i])
    local keko3 = database:get('tshake:'..bot_id..'keko:sudo2'..bot_id..sudo_keko[i])
